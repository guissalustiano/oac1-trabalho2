...
def main():
    # Load load_csvs
    c_O0 = load_csv(Path("c_O0.csv"))
    c_O2 = load_csv(Path("c_O2.csv"))
    java = load_csv(Path("java.csv"))
    pypy = load_csv(Path("pypy.csv"))
    cpython = load_csv(Path("python.csv"))
    ...
    ax.bar(
        ["C -O0", "C -O2", "Java", "PyPy", "CPython"],
        [c_O0['duration'], c_O2['duration'], java['duration'], pypy['duration'], cpython['duration']],
    )
    ...
    ax.bar(
        ["C -O0", "C -O2", "Java", "PyPy", "CPython"],
        [c_O0['instr_per_cycle'], c_O2['instr_per_cycle'], java['instr_per_cycle'], pypy['instr_per_cycle'], cpython['instr_per_cycle']],
    )
